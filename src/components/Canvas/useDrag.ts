 ...
 * }
 */
export function useDrag(targetRef: Ref<HTMLDivElement|undefined>, config?: Reactive<Partial<{
    boundEl?: HTMLElement,
    ...
    function mouseMove($event: MouseEvent) {
        ...
        const target = targetRef.value;
        const {x: bx, y: by, width: bw, height: bh} = wrapEl.getBoundingClientRect(); 
        const {x, y, width, height} = target.getBoundingClientRect();
        const { movementX, movementY } = $event;
        const {enableX, enableY, boundCheck} = newConfig.value;
        const matrix = window.getComputedStyle(target).getPropertyValue('transform');
        let validX = true;
        let validY = true;
        let transformX = 0;
        let transformY = 0;
...
function parseTransformMatrix(matrix: string) {
    ...
    const [a, b, c, d, tx, ty] = matrix.match(/matrix\((.+)\)/)![1].split(',').map(Number);
