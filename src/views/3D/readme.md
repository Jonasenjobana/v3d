...
- 变化矩阵
    ...
    3. 旋转
         ...
         - 任意轴
            ...
            2. 旋转角度...
- 线性相关与线性无关
    - 线性相关 意味着有一组向量能够表示其他任意向量 ( 能够通过平移、缩放、旋转变成另一个)
- 矩阵的秩
    - 行秩和列秩 秩 意味着线性无关的最大个数 (非0行数量)
- 齐次方程和非齐次方程
...
- 迹: 对角线值的合
#### 投影
0. 相机默认放在原点 看向-z方向。（方便计算）相机矩阵变化就是围绕原相机位置 平移旋转至0，0，0原点并看向-Z方向 (games101 session-4讲解)
1. 正交投影
- 将投影包围盒平移到0，0，0原点，再缩放1*1*1变为的正方体 将z值变为0的投影，相当于投影在XYO平面
2. 透视投影
- 将锥视体 挤压变为长方形 然后再做正交投影方式计算。
## 着色器编程
- 本质上是用一套规则 控制不同位置像素显示 变量为法线、uv等
### 套路
1. 颜色取反 1.0 - color
2. fract重复，除了网格重复，还可以偏移到网格中心
3. 采样器texture2d(texture, vec2坐标)；可以内部循环采样 建议部分采样而不是全局贴图采样
### 公式
1. fract
- 保留小数点 常用于归一化、网格划分
- 可作用于向量
...
6. three shader内置变量
    ...
    - 4. gl_FragCoord 屏幕x y 坐标 gl_PointCoord 片元局部坐标 gl_FragColor 片元颜色
7. 正交相机 模拟2D场景、三视图
